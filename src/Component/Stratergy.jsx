...
const Stratergy = (props) => {
  ...
  let [arryleg, setarryleg] = useState([]);
   useEffect(()=>{
    setApiRequest(true)
    setactive("strategy")
    axios.get(`http://192.168.0.125:9000/NSE_company_name/`)
    .then((response)=>{
...
let [peStrike,setpeStrike]=useState([])
  ...
  let getStrikePrice=(e)=>{
    ...
    handlechange(e)
    setApiRequest(false)
    axios.get(`http://192.168.0.125:9000/OptionsData/${e.target.value}/`).then((response)=>{
      console.log(response.data);
...
<option className="" key={id} value={`${value.toUpperCase()}`}>{value} </option>
        ...
        <div className={` ${leg.segments === "FUT" ? "d-none" : ""} `}>
          ...
          <div className={`${leg.segments === "FUT" ? "d-none" : ""}`}>
            ...
            <div className={`${leg.segments === "FUT" ? "d-none" : ""}`}>
            Product type
            <select type="number" className="d-block numberwithoutspin p-2 px-3 border-1 outline-none shadow rounded my-1 w-32  " >

              <option value="Interaday">Interaday</option>
      ...
      {arryleg.map((eachleg, index) => {
        return (
          ...
          >
            ...
            <section className="flex  col-sm-9 justify-between flex-wrap">
              <div className="w-fit">
                <input type="text" className="text-slate-950 p-2 border-1 my-2 w-32 shadow rounded outline-none border-slate-500" value={eachleg.instrument} />
                {/* <select
